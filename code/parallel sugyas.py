...
def get_all_stats(mesorat_hashas):
  ...
  i=0
  for r in tqdm(mesorat_hashas, total=len(mesorat_hashas)):
    i+=1
    if i%1000==0:
        print(i)
    elif i%100==0:
...
def get_all_stats_parallel(mesorat_hashas):
    with mp.Pool() as p:
        results = list(tqdm(p.imap(get_stats_wrapper, mesorat_hashas), total=len(mesorat_hashas)))
...
if __name__ == '__main__':
    ...
    df = get_all_stats(shas_refs)
    with open("../all_shas_stats1.csv", "w") as f:
        df.to_csv(f)